	...
	local paymento = math.random (700,1200)
		...
		if coke >= 1 and success == true then
			...
			local payment = paymentc * removeAmount
				...
				TriggerClientEvent("pNotify:SendNotification", source, {
					text = "You have sold some cocaine for $" .. payment ,
					type = "success",
					progressBar = false,
					queue = "lmao",
					timeout = 2000,
  		...
  		elseif weed >= 1 and success == true then
			...
			local payment = paymentw * removeAmount
  				TriggerClientEvent("pNotify:SetQueueMax", source, "lmao", 5)
				TriggerClientEvent("pNotify:SendNotification", source, {
					text = "You have sold some weed for $" .. payment ,
					type = "success",
					progressBar = false,
					queue = "lmao",
					timeout = 2000,
			...
			local payment = paymentm * removeAmount
  				TriggerClientEvent("pNotify:SetQueueMax", source, "lmao", 5)
				TriggerClientEvent("pNotify:SendNotification", source, {
					text = "You have sold some meth for $" .. payment ,
					type = "success",
					progressBar = false,
					queue = "lmao",
					timeout = 2000,
			...
			TriggerClientEvent("animation", source)
  			...
  			elseif opium >= 1 and success == true then
				...
				local payment = paymento * removeAmount
  					TriggerClientEvent("pNotify:SetQueueMax", source, "lmao", 5)
					TriggerClientEvent("pNotify:SendNotification", source, {
						text = "You have sold some heroin for $" .. payment ,
						type = "success",
						progressBar = false,
						queue = "lmao",
						timeout = 2000,
